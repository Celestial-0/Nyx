...
import { WalletAuth } from "@/components/core/landing/wallet-auth";
...
export function LandingHeader() {
  ...
  const avatarLabel = profile?.displayName?.trim() || profile?.username?.trim() || "User";
        ...
        <div className="relative">
          ...
          {isAuthenticated && menuOpen ? (
            <div className="absolute right-0 mt-2 w-56 rounded-lg border border-border/70 bg-background/95 p-3 shadow-lg backdrop-blur">
              ...
              <p className="mt-1 text-xs text-muted-foreground">{shortWallet}</p>
              <Button
                type="button"
                variant="outline"
