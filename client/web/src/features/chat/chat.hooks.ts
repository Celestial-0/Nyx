...
import { startDirectConversation } from "@/features/dm/dm.api"
import {
...
import { createChatSocketClient, handleChatRealtimeEvent } from "./chat.ws"
import type {
  ChatConversation,
...
function getContactAlias(userId: string | null | undefined) {
  ...
  return (
    useContactsStore
      .getState()
      .contacts.find((contact) => contact.user.id === userId)?.alias ?? null
  )
}
...
function getConversationSearchValue(summary: ChatConversationSummary) {
  ...
  const groupValues =
    summary.groupState == null
      ? []
      : [
          String(summary.groupState.memberCount),
          summary.groupState.senderKeyState?.status ?? "",
        ]
...
function mapConversation(input: {
  ...
  const { summary } = input
  const localDevice = input.walletAddress
    ? getStoredLocalAuthDevice(input.walletAddress)
    : null
  ...
  return {
    ...
    avatarSrc: getAvatarUrl(
      summary.directPeer?.walletAddress ??
        summary.directPeer?.userId ??
        summary.id
    ),
    lastMessageAt: summary.lastMessageAt,
    lastMessageKind: summary.lastMessageKind,
    lastMessagePreview:
      decryptedLastMessage?.text ?? getEncryptedPreview(summary.lastMessageKind),
...
function mapHistoryMessage(input: {
  ...
  const localDevice = input.walletAddress
    ? getStoredLocalAuthDevice(input.walletAddress)
    : null
  const decrypted = input.currentUserId && input.walletAddress
    ? decryptConversationEnvelope({
        ...
        senderKeyState:
          input.context?.senderKeyState ?? input.conversation.senderKeyState,
...
function mapRoomMember(member: RoomMember): ChatMember {
  return {
    id: member.userId,
    ...
    avatarSrc: getAvatarUrl(member.walletAddress),
...
function buildSelfMember(input: {
  ...
  return {
    ...
    avatarSrc: getAvatarUrl(input.walletAddress),
  ...
  try {
    ...
    const activeConversation =
      useChatStore.getState().conversations.find(
        (conversation) =>
          conversation.id === useChatStore.getState().activeConversationId
      ) ?? null
...
function syncComposerForConversation(conversationId: string | null) {
  ...
  if (!conversation || !authUser) {
    chat.setComposerState({
      locked: true,
      notice: "Your encrypted session is not ready yet.",
    })
    return
  }
  ...
  if (!localDevice) {
    chat.setComposerState({
      locked: true,
      notice:
        "This browser is missing its local chat device keys. Sign in again to restore encrypted messaging.",
    })
    return
  }
  ...
  if (!context || !members.length) {
    chat.setComposerState({
      locked: true,
      notice:
        "Load room members and sender-key state before sending encrypted group messages.",
    })
    return
  }
  ...
  if (!context.senderKeyState) {
    chat.setComposerState({
      locked: true,
      notice: "This group room has no sender-key state yet.",
    })
    return
  }
  ...
  const resolvedSenderKey = ensureResolvedGroupSenderKey({
    ...
    senderDeviceId:
      context.senderKeyState.createdByDeviceId ??
      localDevice.registration.deviceId,
    distribution: null,
...
export async function loadConversationListAction(options?: {
  background?: boolean
}) {
  const chat = useChatStore.getState()
  const authUser = useAuthStore.getState().user
  ...
  try {
    const accessToken = await requireAccessToken()
    const response = await getConversationList(accessToken)
...
export async function loadOlderConversationHistoryAction(
  conversation: ChatConversation
) {
  const chat = useChatStore.getState()
  ...
  try {
    ...
    const response = await getConversationHistory({
      ...
      query: {
        ...
        beforeMessageId: pageInfo.nextBeforeMessageId,
      },
    })
    const members =
      conversation.type === "group"
...
export async function loadConversationDetailsAction(
  ...
  try {
    ...
    if (conversation.type === "direct") {
      ...
      if (conversation.directPeer) {
        members.push({
          ...
          role: "Direct contact",
          memberRole: null,
          status: conversation.directPeer.deviceBundles.length
            ? "verified"
            : "limited",
          avatarSrc: getAvatarUrl(conversation.directPeer.walletAddress),
    ...
    const [roomDetail, roomMembers] = await Promise.all([
      getRoom(conversation.id, accessToken),
      getRoomMembers(conversation.id, accessToken),
    ])
...
async function refreshConversationStateAction(conversationId: string) {
  const chat = useChatStore.getState()
  await loadConversationListAction({ background: true })
  ...
  await loadConversationDetailsAction(refreshedConversation, {
    background: true,
  })
}
...
async function ensureGroupEnvelopeState(conversation: ChatConversation) {
  ...
  if (context.senderKeyState.distributionRequired) {
    ...
    return {
      senderKey: distribution.senderKey,
      distribution,
    }
  }
...
async function sendEncryptedMessageAction(input: {
  conversation: ChatConversation
  text: string
}) {
  const chat = useChatStore.getState()
  ...
  if (refreshedConversation.type === "direct") {
    ...
    const { senderKey, distribution } = await ensureGroupEnvelopeState(
...
export async function searchDirectoryUsersAction(query: string) {
  const accessToken = await requireAccessToken()
  return searchUsers(query, accessToken)
}
...
export async function startDirectConversationAction(input: {
  ...
  const accessToken = await requireAccessToken()
  const authUser = useAuthStore.getState().user
  const response = await startDirectConversation(input, accessToken)
...
export async function createGroupConversationAction() {
  ...
  const authUser = useAuthStore.getState().user
  const accessToken = await requireAccessToken()
  const response = await createGroupRoom({ type: "group" }, accessToken)
  const summary: ChatConversationSummary = {
    ...
    groupState: {
      memberCount: 1,
      senderKeyState: response.senderKeyState,
    },
  }
...
export async function saveContactAliasAction(input: {
  ...
  const contact = await saveContact(input, accessToken)
...
export async function updateContactAliasAction(
  contactUserId: string,
  alias: string | null
) {
  const accessToken = await requireAccessToken()
  ...
  const contact = await updateContactAlias(contactUserId, alias, accessToken)
  contacts.upsertContact(contact)
...
export async function removeContactAliasAction(contactUserId: string) {
  ...
  await removeContact(contactUserId, accessToken)
...
export async function toggleConversationMuteAction(
  conversation: ChatConversation
) {
  const accessToken = await requireAccessToken()
  const chat = useChatStore.getState()
  ...
  const response = await updateRoomMute(conversation.id, mutedUntil, accessToken)
...
export async function hideMessageAction(message: ChatMessage) {
  const accessToken = await requireAccessToken()
  const chat = useChatStore.getState()
  await hideMessageForMe(message.id, accessToken)
...
export async function deleteMessageAction(message: ChatMessage) {
  const accessToken = await requireAccessToken()
  const chat = useChatStore.getState()
  await deleteMessageForEveryone(message.id, accessToken)
  chat.removeMessage(message.conversationId, message.id)
  const conversation =
    chat.conversations.find((item) => item.id === message.conversationId) ?? null
  if (conversation) {
...
export async function updateGroupMemberRoleAction(input: {
  ...
  const accessToken = await requireAccessToken()
  const chat = useChatStore.getState()
  await updateRoomMemberRole(input.roomId, input.userId, input.role, accessToken)

  const conversation =
    chat.conversations.find((item) => item.id === input.roomId) ?? null
...
export function useChatRealtime() {
  ...
  const subscribedConversationIds = useChatStore(
    (state) => state.subscribedConversationIds
  )
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      send({
        ...
        data: {},
      } satisfies ChatSocketRequest<Record<string, never>>)
    }, 25_000)
  ...
  useEffect(() => {
    ...
    for (const conversationId of missingConversationIds) {
      ...
      } satisfies ChatSocketRequest<{ conversationId: string }>)
    }
...
export function useChatProfileSync() {
  ...
  const displayName =
    profile?.displayName?.trim() ||
    profile?.username?.trim() ||
    (authUser ? getShortWallet(authUser.walletAddress) : "Nyx User")
  const avatarSeed =
    profile?.walletAddress ?? authUser?.walletAddress ?? displayName
...
export function useChatShell() {
  ...
  const historyStateByConversation = useChatStore(
    (state) => state.historyStateByConversation
  )
  ...
  const historyErrorByConversation = useChatStore(
    (state) => state.historyErrorByConversation
  )
  const membersByConversation = useChatStore(
    (state) => state.membersByConversation
  )
  const contextByConversation = useChatStore(
    (state) => state.contextByConversation
  )
  const detailsStateByConversation = useChatStore(
    (state) => state.detailsStateByConversation
  )
  const detailsErrorByConversation = useChatStore(
    (state) => state.detailsErrorByConversation
  )
  const typingByConversation = useChatStore(
  ...
  const peerOnline = useMemo(() => {
    ...
    const isOnline = peerId ? onlineUserIds.has(peerId) : null
    console.log("[DEBUG] peerOnline check:", {
      peerId,
      isOnline,
  ...
  useEffect(() => {
    ...
    if (
      (historyStateByConversation[activeConversation.id] ?? "idle") === "idle"
    ) {
      void loadConversationHistoryAction(activeConversation)
    }

    if (
      (detailsStateByConversation[activeConversation.id] ?? "idle") === "idle"
    ) {
  ...
  useEffect(() => {
    ...
    for (const message of activeMessages) {
      if (
        ...
        readAckedMessagesRef.current.has(message.id)
      ) {
        continue
      }
  ...
  useEffect(() => {
    ...
    const requiresSenderKeySync =
      activeContext?.senderKeyState?.distributionRequired === true
    ...
    const lastRefreshAt =
      placeholderRefreshAtRef.current[activeConversation.id] ?? 0
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await sendEncryptedMessageAction({
      ...
      if (
        ...
        openPaymentsPanelAction({
          source: "message-send",
          recoveryMessage:
            "Add credits to keep sending messages in this conversation.",
        })
      }
    ...
    try {
      ...
      if (
        ...
        openPaymentsPanelAction({
          source: "group-create",
          recoveryMessage:
            "Recharge your balance before creating another group room.",
        })
      }
