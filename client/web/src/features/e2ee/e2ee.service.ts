...
export async function getOrCreateLocalDevice(input: {
  ...
  const legacyRegistration = readStoredDeviceRegistration(input.walletAddress)

  if (legacyRegistration) {
    clearStoredLocalDevice(input.walletAddress)
  }
...
function encryptWithSymmetricKey(input: {
  key: Uint8Array
