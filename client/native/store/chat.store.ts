...
/**
 * Chat store. Ported 1:1 from the web client's `features/chat/chat.store.ts`
 * (no browser APIs are used, so it transfers unchanged aside from imports).
 */
...
type ChatActions = {
  ...
  patchConversation: (conversationId: string, patch: Partial<ChatConversation>) => void;
  ...
  setContext: (conversationId: string, context: ChatConversationContext | null) => void;
  ...
  setTypingPresence: (conversationId: string, userId: string, isTyping: boolean) => void;
  ...
  setComposerState: (input: { locked: boolean; notice: string | null }) => void;
  setRealtimeSender: (value: ((payload: ChatSocketRequest) => boolean) | null) => void;
...
export const useChatStore = create<ChatStore>((set) => ({
  ...
  setConversations: (conversations) =>
    set((state) => {
      const unreadById = new Map(
        state.conversations.map((conversation) => [conversation.id, conversation.unreadCount])
      );

      const nextConversations = sortConversations(
        conversations.map((conversation) => ({
          ...conversation,
          unreadCount: unreadById.get(conversation.id) ?? conversation.unreadCount,
          lastMessagePreview:
            conversation.lastMessagePreview || encryptedPreview(conversation.lastMessageKind),
          lastMessagePreviewFallback:
            conversation.lastMessagePreviewFallback ?? !conversation.lastMessagePreview,
          lastActivityLabel:
            conversation.lastActivityLabel || formatRelativeLabel(conversation.lastMessageAt),
        }))
      );

      const activeConversationId =
        nextConversations.find((conversation) => conversation.id === state.activeConversationId)
          ?.id ??
        ...
        state.conversations.map((conversation) =>
          conversation.id === conversationId
            ? {
                ...
                ...patch,
                lastMessagePreviewFallback:
                  patch.lastMessagePreviewFallback ?? conversation.lastMessagePreviewFallback,
                lastActivityLabel:
                  patch.lastActivityLabel ??
                  formatRelativeLabel(patch.lastMessageAt ?? conversation.lastMessageAt),
              }
            : conversation
        )
      ),
  ...
  setMessages: (conversationId, messages, pageInfo) =>
    set((state) => ({
      ...
      historyPageInfoByConversation: pageInfo
        ? { ...state.historyPageInfoByConversation, [conversationId]: pageInfo }
  ...
  setContext: (conversationId, context) =>
    set((state) => ({
      contextByConversation: { ...state.contextByConversation, [conversationId]: context },
      detailsStateByConversation: {
          ...
          state.conversations.map((conversation) =>
            conversation.id === message.conversationId
              ? {
                  ...
                  unreadCount:
                    isOwnMessage || !markUnread ? 0 : conversation.unreadCount + 1,
                }
              : conversation
          )
        ),
  ...
  setComposerState: ({ locked, notice }) =>
    set({ composerLocked: locked, composerNotice: notice }),
  ...
  removeConversation: (conversationId) =>
    set((state) => {
      ...
      const { [conversationId]: _historyState, ...historyStateByConversation } =
        state.historyStateByConversation;
      const { [conversationId]: _olderHistoryState, ...olderHistoryStateByConversation } =
        state.olderHistoryStateByConversation;
