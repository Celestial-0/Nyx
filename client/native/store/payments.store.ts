...
/** Payments/credits store. Ported from web `features/payments/payments.store.ts`. */
...
type PaymentsActions = {
  ...
  patchSnapshot: (
    updater: (snapshot: PaymentCreditsSnapshot) => PaymentCreditsSnapshot
  ) => void;
...
export const usePaymentsStore = create<PaymentsStore>((set) => ({
  ...
  patchSnapshot: (updater) =>
    set((state) => ({ snapshot: state.snapshot ? updater(state.snapshot) : state.snapshot })),
